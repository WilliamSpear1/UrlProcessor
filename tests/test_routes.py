...
def test_task_status_pending(client):
    ...
    with patch('app.celery_app.AsyncResult') as mock_async:
...
def test_download_success(client):
    ...
    with patch('app.fetch_urls') as mock_fetch:
...
def test_download_not_registered(client):
    ...
    with patch('app.fetch_urls.delay', side_effect=Exception('Task Not Registered')):
        resp = client.post('/fetch-urls', json={'url': 'http://example.com'})
        data = resp.get_json()
